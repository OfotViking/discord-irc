...
const REQUIRED_FIELDS = ['server', 'nickname', 'channelMapping', 'roleMapping', 'discordToken'];
...
class Bot {
  constructor(options) {
    ...
    validateRoleMapping(options.roleMapping);
    ...
    this.channels = _.values(options.roleMapping);
    ...
    _.forOwn(options.roleMapping, (ircChan, discordChan) => {
      this.roleMapping[discordChan] = ircChan.split(' ')[0];
    });
    this.invertedMapping = _.invert(this.channelMapping);
    this.invertedRoleMapping = _.invert(this.roleMapping); //maybe not needed
    ...
    if (ircChannel) {
      ...
      if (this.isCommandMessage(text)) {
        ...
        function noPermission() {
          message.channel.sendMessage("You got no permission to do that, do you?!");
        };
        
        function noRole() {
          message.channel.sendMessage("Missing Discord role for this command");
        };
        
        const parsed = message.content.split(" ");
        const command = parsed[0];
        const discordRole = this.roleMapping[command];
        
        if (!discordRole) {
          return;
        };
        
        logger.debug('Command to Role mapping:', command, discordRole);
        if (discordRole == "everyone") {
          var role = "everyone";
          } else {
          var role = message.guild.roles.find("name", discordRole);
        }
        ...
        if ((message.member.roles.has(role.id)) || (role == "everyone")) {
